...
- **Venue-aware parsing** — recognises messages from Smart Trade, Bloomberg FXGO (FIXBOOK, Algo/FXOM, STP), Bloomberg DOR, 360T (RFS Market Taker + SUN swaps order book + TradeImporter), SGX Titan OTC, and LSEG / Refinitiv FX Matching (MAPI), including their proprietary custom tags
...
| Bloomberg FXGO | Spot execution, NDF with fixing information, multi-symbol quote request, an LP-side FIXBOOK RFS swap quote (far leg all-in in BidPx2/OfferPx2) and swap fill (LastPx2), an Algo (FXOM) done-for-day report, and an STP drop-copy FX option with premium, hedge, reference prices and competing dealer quotes |
...
| **Bloomberg FXGO** | Bloomberg's FX trading *platform* — the front-end where FX price discovery and execution happen. Covers the three plain-FIX 4.4 FXGO interfaces (Bloomberg CompID `BLP`): **FIXBOOK for Liquidity Providers** (streaming / RFS / batch-RFQ quoting, market data, orders and executions), **FXGO Algo (FXOM)** (algo order routing with strategy parameters and daily-average fills), and **FXGO STP** (post-trade drop copy and allocation notification for all FX products including FX options — barriers, TARFs, premium, hedge, reference prices). Swap economics come from the FXGO flat-tag shape: near all-in in 132/133 or 31, far all-in in BidPx2 (6050) / OfferPx2 (6051) or LastPx2 (6160), with swap points derived far − near |
